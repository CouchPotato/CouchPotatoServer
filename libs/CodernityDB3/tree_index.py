...
# twolvl_cache(self._read_single_leaf_record)
    ...
    def _read_single_leaf_record(self, leaf_start, key_index):
        ...
        key, doc_id, start, size, status = struct.unpack(b'<' + str(self.single_leaf_record_format).encode(), data)
...
#                    should't happen, crashes earlier on id index
    ...
    def _update_element(self, leaf_start, key_index, new_data):
        ...
        self.buckets.write(struct.pack(b'<' + fixed_meta_format,
                                       *new_data))
...
#                    should't happen, crashes earlier on id index
    ...
    def _find_key_in_leaf_using_binary_search(self, key, leaf_start, nr_of_elements, doc_id=None, mode=None, return_closest=False):
        ...
        candidate_start, candidate_index, move_buffer = \
            self._choose_next_candidate_index_in_leaf(leaf_start,
        ...
        if doc_id and doc_id == curr_doc_id and curr_status == 'o':
            ...
            if doc_id is not None and doc_id != curr_doc_id:
                leaf_start, nr_of_elements, matching_record_index, curr_status = \
                    self._match_doc_id(doc_id,
                                       key,
                                       matching_record_index,
                                       leaf_start,
                                       nr_of_elements)
                curr_key, curr_doc_id, curr_start, curr_size, curr_status = \
                    self._read_single_leaf_record(leaf_start,
                                                  matching_record_index)
    ...
    def _find_place_in_leaf_using_binary_search(self, key, leaf_start, nr_of_elements):
        ...
        imin, imax = 0, nr_of_elements - 1
        buffer_start, buffer_end = self._set_buffer_limits()
        candidate_start, candidate_index, move_buffer = self.\
            _choose_next_candidate_index_in_leaf(leaf_start,
                                                 self._calculate_key_position(leaf_start,
                                                                              (imin + imax) / 2,
                                                                              'l'),
                                                 buffer_start,
    ...
    def _update_leaf_ready_data(self, leaf_start, start_index, new_nr_of_elements, records_to_rewrite):
        ...
        self.buckets.write(
            struct.pack(
                b'<' + str((new_nr_of_elements - start_index) *
                self.single_leaf_record_format).encode(),
                *records_to_rewrite))
...
#        self._read_single_leaf_record.delete(leaf_start)
    ...
    def _update_leaf(self, leaf_start, new_record_position, nr_of_elements,
        ...
        if nr_of_records_to_rewrite == 0:  # just write at set position
            ...
            self.buckets.write(
                struct.pack(
                    b'<' + str((nr_of_records_to_rewrite +
                                1) * self.single_leaf_record_format).encode(),
                    new_key,
                    new_doc_id,
                    new_start,
                    new_size,
        ...
        if not on_deleted:  # when new record replaced deleted one, nr of leaf elements stays the same
            self.buckets.write(struct.pack(b'<h', nr_of_elements + 1))
...
#        self._read_single_leaf_record.delete(leaf_start)
    ...
    def _update_leaf_size_and_pointers(self, leaf_start, new_size, new_prev, new_next):
        ...
        self.buckets.write(
            struct.pack(
                b'<' + str(self.elements_counter_format + 2 * self.pointer_format).encode(),
                new_size,
                new_prev,
                new_next))
    ...
    def _update_leaf_prev_pointer(self, leaf_start, pointer):
        self.buckets.seek(leaf_start + self.elements_counter_size)
        self.buckets.write(struct.pack(b'<' + str(self.pointer_format).encode(),
                                       pointer))
    ...
    def _update_size(self, start, new_size):
        self.buckets.seek(start)
        self.buckets.write(struct.pack(b'<' + str(self.elements_counter_format).encode(),
                                       new_size))
    ...
    def _create_new_root_from_leaf(self, leaf_start, nr_of_records_to_rewrite, new_leaf_size, old_leaf_size, half_size, new_data):
        ...
        if nr_of_records_to_rewrite > half_size:
            ...
            left_leaf_data = struct.pack(b'<' + str(self.leaf_heading_format + self.single_leaf_record_format
                                         * (self.node_capacity - nr_of_records_to_rewrite)).encode(),
                                         old_leaf_size,
                                         0,
                                         right_leaf_start_position,
                                         *leaf_data[:-nr_of_records_to_rewrite * 5])
            left_leaf_data += struct.pack(
                b'<' + str(self.single_leaf_record_format * (
                    nr_of_records_to_rewrite - new_leaf_size + 1)).encode(),
                new_data[0],
                new_data[1],
                new_data[2],
                new_data[3],
            ...
            right_leaf_data = struct.pack(b'<' + str(self.elements_counter_format + 2 * self.pointer_format +
                                          self.single_leaf_record_format *
                                          new_leaf_size).encode(),
                                          new_leaf_size,
                                          left_leaf_start_position,
                                          0,
            ...
            right_leaf_data = struct.pack(b'<' + str(self.elements_counter_format + 2 * self.pointer_format +
                                          self.single_leaf_record_format * (new_leaf_size -
                                                                            nr_of_records_to_rewrite - 1)).encode(),
                                          new_leaf_size,
                                          left_leaf_start_position,
                                          0,
                                          *records_before)
    ...
    def _split_leaf(
        ...
        if create_new_root:  # leaf is a root
            ...
            if nr_of_records_to_rewrite > half_size:  # insert key into first half of leaf
                ...
                new_leaf = struct.pack(b'<' + str(self.elements_counter_format + 2 * self.pointer_format +
                                       self.single_leaf_record_format *
                                       new_leaf_size).encode(),
                                       new_leaf_size,
                                       leaf_start,
...
#                self._read_single_leaf_record.delete(leaf_start)
    ...
    def _create_new_root_from_node(self, node_start, children_flag, nr_of_keys_to_rewrite, new_node_size, old_node_size, new_key, new_pointer):
            ...
            if nr_of_keys_to_rewrite == new_node_size:
                ...
                left_node = struct.pack(b'<' + str(self.node_heading_format + self.pointer_format +
                                        old_node_size * (self.
                                                         key_format + self.pointer_format)).encode(),
                                        old_node_size,
                                        children_flag,
                                        *old_node_data[:old_node_size * 2 + 1])

                right_node = struct.pack(b'<' + str(self.node_heading_format + self.pointer_format +
                                         new_node_size * (self.
                                                          key_format + self.pointer_format)).encode(),
                                         new_node_size,
                                         children_flag,
...
#               'inserting key into second half of node and creating new root'
                ...
                right_node += struct.pack(
                    b'<' + str((nr_of_keys_to_rewrite + 1) *
                    (self.key_format + self.pointer_format)).encode(),
                    new_key,
                    new_pointer,
                    *keys_after)
    ...
    def _split_node(self, node_start, nr_of_keys_to_rewrite, new_key, new_pointer, children_flag, create_new_root=False):
        ...
        if create_new_root:
            ...
            if nr_of_keys_to_rewrite == new_node_size:  # insert key into first half of node
                ...
                new_node = struct.pack(b'<' + str(self.node_heading_format + self.pointer_format +
                                       (self.key_format +
                                        self.pointer_format) * new_node_size).encode(),
                                       new_node_size,
                                       children_flag,
                                       new_pointer,
                                       *old_node_data)
                ...
                new_node_start = self.buckets.tell()
                # prepare new node_data
                new_node = struct.pack(b'<' + str(self.node_heading_format +
                                       self.pointer_format + (self.key_format +
                                                              self.pointer_format) * new_node_size).encode(),
                                       new_node_size,
                                       children_flag,
                ...
                self.buckets.write(
                    struct.pack(
                        b'<' + str((self.key_format + self.pointer_format) *
                        (nr_of_keys_to_rewrite - new_node_size)).encode(),
                        new_key,
                        new_pointer,
                ...
                new_node = struct.pack(b'<' + str(self.node_heading_format + self.pointer_format +
                                       (self.key_format + self.pointer_format) *
                                       (new_node_size -
                                        nr_of_keys_to_rewrite - 1)).encode(),
                                       new_node_size,
                                       children_flag,
                                       first_leaf_pointer,
                                       *keys_before)
                new_node += struct.pack(b'<' + str((self.key_format + self.pointer_format) *
                                        (nr_of_keys_to_rewrite + 1)).encode(),
                                        new_key,
                                        new_pointer,
                                        *keys_after)
    ...
    def insert_first_record_into_leaf(self, leaf_start, key, doc_id, start, size, status):
        ...
        self.buckets.write(struct.pack(b'<' + self.single_leaf_record_format.encode(),
                                       key,
                                       doc_id,
                                       start,
                                       size,
...
#            self._read_single_leaf_record.delete(leaf_start)
    ...
    def _update_node(self, new_key_position, nr_of_keys_to_rewrite, new_key, new_pointer):
        if nr_of_keys_to_rewrite == 0:
            ...
            self.buckets.write(
                struct.pack(b'<' + str(self.key_format + self.pointer_format).encode(),
                            new_key,
                            new_pointer))
            self.flush()
        else:
            ...
            keys_to_rewrite = struct.unpack(
                b'<' + str(nr_of_keys_to_rewrite * (self.key_format + self.pointer_format)).encode(), data)
            self.buckets.seek(new_key_position)
            self.buckets.write(
                struct.pack(
                    b'<' + str((nr_of_keys_to_rewrite + 1) *
    ...
    def _find_key_to_update(self, key, doc_id):
        ...
        nr_of_elements, prev_leaf, next_leaf = self._read_leaf_nr_of_elements_and_neighbours(containing_leaf_start)
        try:
            leaf_start, record_index, doc_id, l_key, start, size, status = \
                self._find_key_in_leaf_for_update(key,
                                                  doc_id,
                                                  containing_leaf_start,
                                                  nr_of_elements)
            ## print("#####", "_find_key_in_leaf_for_update") # TODO
        except ElemNotFound:
            if next_leaf:
                nr_of_elements = self._read_leaf_nr_of_elements(next_leaf)
            else:
